...
const generator = async (sitemap) => {
  ...
  if (typeof linkResolver !== "function") {
    throw new Error(
      "The linkResolver function is undefined, this is needed to build sitemap links"
    );
  }

  if (accessToken !== null && accessToken.length <= 1) {
    throw new Error(
      "The API Access token appears incorrect, please double check as it is short."
    );
  }
  ...
  /**
   * gets all documents of type doc and pushes it to docs.
   * @param {*} doc
   */
  const populatePaths = async (doc) => {
    const { results } = await api.query(
      Prismic.Predicates.at("document.type", doc),
      {
        lang: "*",
        pageSize: 10000, // to get all pages
      }
    );
    docs.push(...results);
    return resolvePromise(doc);
  };

  const getData = async () => {
    return Promise.all(documentTypes.map((item) => populatePaths(item)));
  };

  return getData().then(async () => {
